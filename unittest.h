 ...
 *
 * range() and  arrayOfLength() can be used to describe sequences that can be
 * used with any of the container objects.
 *
 *     assertThat(range(v.begin(), v.end()), hasItem(z));
 *     assertThat(arrayOfLength(array, len), hasItem(z));
 *
 * The common case of a sequence consisting of some container's entire contents
 * from begin() to end() can also be written as contentsOf():
 *
 *     assertThat(contentsOf(v), hasItem(z));
 *
 *     assertThat(x, isIn(v));
 ...
 *
 * A test for equality of corresponding elements between two sequences can be
 * made with matches().
 *
 *     assertThat(range(v.begin(), v.end()), matches(range(L.begin(), L.end())));
 *
 * This can also be written as
 *
 *     assertThat(contentsOf(v), matches(contentsOf(L)));
 *
 * ## Combining Matchers
 *
...
template <typename T>
CppUnitLite::IteratorRange<typename T::const_iterator> contentsOf (const T& container)
{
	return CppUnitLite::IteratorRange<typename T::const_iterator>(container.begin(), container.end());
}
...
template <typename T>
CppUnitLite::MatchesMatcher<const T*> matches (CppUnitLite::IteratorRange<const T*> arrayrange)
{
	return CppUnitLite::MatchesMatcher<const T*>(arrayrange);
}
template <typename Iterator>
CppUnitLite::MatchesMatcher<Iterator> matches (CppUnitLite::IteratorRange<Iterator> range)
{
	return CppUnitLite::MatchesMatcher<Iterator>(range);
}
